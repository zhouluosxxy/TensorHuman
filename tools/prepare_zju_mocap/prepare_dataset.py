...
def compute_continuous_time(joints_list):
    """直接返回图片索引的整数数组 [0, 1, 2, ..., N-1]"""
    N = len(joints_list)  # 帧数
    t_idx = np.arange(N)  # 生成整数索引数组
    return t_idx
...
def main(argv):
    ...
    for idx, ipath in enumerate(tqdm(img_paths)):
        if subject in ['313', '315']:
            _, image_basename, _ = split_path(ipath)
    ...
    # 基于索引生成整数时间
    t_idx_values = compute_continuous_time(joints_list)
    ...
    for idx, ipath in enumerate(tqdm(img_paths)):
        ...
        mesh_infos[out_name] = {
            ...
            'time': int(t_idx_values[idx])  # 存储整数索引
        }
