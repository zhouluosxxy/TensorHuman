...
class Network(nn.Module):
    ...
    def _query_mlp(
        ...
        if 'loss_reg' in result:
            output['loss_reg'] = result['loss_reg']
        return output
    ...
    def _expand_input(input_data, total_elem):
        ...
        assert input_data.shape[0] == 1, f"Expected shape[0] == 1, got {input_data.shape}"
        ...
        for i in range(0, pos_flat.shape[0], chunk):
            ...
            if  not cfg.ignore_non_rigid_motions:
                ...
                result = self.non_rigid_mlp(
                    ...
                    time=self._expand_input(time, total_elem)
                )
                xyz = result['xyz']
            ...
            raws.append(cnl_result['rgb_sigma'])  # CanonicalMLP 返回 rgb_sigma
            if 'loss_reg' in cnl_result:
                loss_reg_list.append(cnl_result['loss_reg'])  # 收集 loss_reg

        output = {}
        output['raws'] = torch.cat(raws, dim=0).to(cfg.primary_gpus[0])
        if loss_reg_list:  # 如果有 loss_reg，计算平均值
            output['loss_reg'] = torch.stack(loss_reg_list).mean()
        return output

    def _batchify_rays(self, rays_flat, time=None,iter_val=None, **kwargs):
        all_ret = {}
        for i in range(0, rays_flat.shape[0], cfg.chunk):
            ret = self._render_rays(rays_flat[i:i + cfg.chunk], time=time,iter_val=iter_val, **kwargs)
            for k in ret:
                if k not in all_ret:
                    all_ret[k] = []
                all_ret[k].append(ret[k])

        all_ret = {k: torch.cat(all_ret[k], 0) if k != 'loss_reg' else torch.stack(all_ret[k]).mean()
                   for k in all_ret}
        return all_ret
    ...
    def _raw2outputs(raw, raw_mask, z_vals, rays_d, bgcolor=None):
        ...
        infinity_dists = torch.Tensor([1e10]).expand(dists[..., :1].shape).to(dists)
        ...
        weights = alpha * torch.cumprod(
            torch.cat([torch.ones((alpha.shape[0], 1)).to(alpha), 1. - alpha + 1e-10], dim=-1), dim=-1)[:, :-1]
    ...
    def _render_rays(
        ...
        ret = {'rgb': rgb_map,
               'alpha': acc_map,
               'depth': depth_map}
        # 如果 query_result 中包含 loss_reg，则添加到返回结果
        if 'loss_reg' in query_result:
            ret['loss_reg'] = query_result['loss_reg']
        return ret
    ...
    def forward(self,
        ...
        # correct body pose
        if 1==10 : #  iter_val >= cfg.pose_decoder.get('kick_in_iter', 0):
        ...
        all_ret = self._batchify_rays(packed_ray_infos, time=time, iter_val=iter_val,**kwargs)
        for k in all_ret:
            k_shape = list(rays_shape[:-1]) + list(all_ret[k].shape[1:]) if k != 'loss_reg' else []
            if k != 'loss_reg':
                all_ret[k] = torch.reshape(all_ret[k], k_shape)
            # loss_reg 保持标量形式，不需要 reshape
